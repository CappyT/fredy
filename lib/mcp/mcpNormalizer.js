...
export function normalizeListListings(queryResult, { page, pageSize }) {
  ...
  if (listings.length > 0) {
    ...
    for (const l of listings) {
      ...
      md += `| ${cell(l.id)} | ${cell(l.title)} | ${cell(l.address)} | ${cell(formatAmount(l.price, l.currency))} | ${dealTypeLabel(l.dealType)} | ${cell(l.size)} | ${cell(l.provider)} | ${cell(internetCell(l))} | ${l.is_active ? 'yes' : 'no'} | ${cell(l.status?.status)} | ${l.published_at != null ? formatDate(l.published_at) : '–'} | ${formatDate(l.created_at)} | ${cell(l.job_name)} |\n`;
    }
...
export function normalizeGetListing(listing) {
  ...
  md += `- **Price:** ${formatAmount(listing.price, listing.currency) ?? '–'}${listing.dealType === 'rent' ? ' (monthly rent)' : ''}\n`;
