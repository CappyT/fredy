...
import { getStoredListingPhoto } from './storedListingPhoto.js';
import { isDefaultCurrency, normalizeCurrency } from '../utils/currency.js';
...
export function createMcpServer() {
  ...
  server.tool(
    ...
    async ({ listingId }, extra) => {
      ...
      const storedPhoto = getStoredListingPhoto(listingId);
  ...
  server.tool(
    ...
    async (args, extra) => {
      ...
      if (args.listingId) {
        ...
        // Both arms below budget in euros, so a price in another currency would be scored as if it were one.
        if (!isDefaultCurrency(listing.currency)) {
          return normalizeError(
            `That listing is priced in ${normalizeCurrency(listing.currency)}, and the financing model only covers prices in EUR.`,
            'calculate_financing',
          );
        }
