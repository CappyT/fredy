...
import { queryListings, getListingById, getListingImage } from '../services/storage/listingsStorage.js';
 ...
 *
 * Both sources of a photograph need this: a portal serving `application/octet-stream` and a stored
 * blob whose recorded content type the portal got wrong are the same problem, and a vision model
 * refuses a mime type it does not know. Returns null when the bytes are of no format a model reads,
 * which is the caller's cue to say so rather than to hand over something unusable.
 *
 * @param {string|null|undefined} declaredMimeType What the portal or the stored row calls it.
...
export function createMcpServer() {
  ...
  server.tool(
    ...
    async ({ listingId }, extra) => {
      ...
      // The bytes Fredy kept when it stored the listing come first, exactly as the image route
      // serves them. The url on the row is a rental - idealista signs its cloudfront links for
      // about a day - so fetching it was guaranteed to answer 403 on precisely the adverts this
      // instance already holds a photograph of.
      const stored = getListingImage(listingId);
      if (stored?.bytes != null && stored.bytes.length >= IMAGE_HEADER_BYTES) {
        const storedMime = resolveImageMimeType(stored.mime_type, stored.bytes);
        if (storedMime != null) {
          return {
            content: [{ type: 'image', data: Buffer.from(stored.bytes).toString('base64'), mimeType: storedMime }],
          };
        }
      }
      ...
      if (bytes.length < IMAGE_HEADER_BYTES) {
        return normalizeError(
      ...
      const resolvedMime = resolveImageMimeType(headerMimeType, bytes);
