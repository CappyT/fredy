 ...
 *
 * The endpoint refuses a request without a `datadome` cookie with 403 and names the challenge, and
 * which challenge it names depends on the exit address. `readThroughGuard` owns both remedies: the
 * read is asked again from another exit first, and only then is a solvable challenge handed to
 * `lib/services/datadome.js`, whose cooldown is what keeps a search the endpoint keeps refusing
 * from paying per page.
 *
...
async function requestPage(body) {
  ...
  if (!answer.ok) {
    ...
    logger.error(
      `ImmoScout24.ch answered ${answer.status} ${answer.statusText}: ${answer.body.slice(0, 300)}`.trimEnd(),
    );
    return null;
  }
