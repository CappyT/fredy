 ...
 * The endpoint answers 25 adverts at a time and counts the pages, so a run reads them all.
 *
 ...
 */
async function requestApiPage(url, page) {
  const endpoint = new URL(convertMapSearchToApi(url));
  endpoint.searchParams.set(PAGE_PARAM, String(page));
...
async function getListingsFromApi(url) {
  ...
  for (let page = 1; page <= MAX_PAGES; page++) {
    ...
    const answer = await requestApiPage(url, page);
    if (answer == null) break;
    results.push(...answer.results);

    if (page >= Math.min(answer.maxPages, MAX_PAGES)) {
      if (answer.maxPages > MAX_PAGES) {
 ...
 * @param {import('puppeteer').Browser} browser the shared browser of the current job run
 * @returns {Promise<any[]>} the raw results: every page of a map search, the first of a town one
 */
