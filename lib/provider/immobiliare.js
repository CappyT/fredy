...
/** Where the android app asks for one property, which is the one place the dates live. */
...
/**
 * Read the date the portal itself attaches to an advert, out of the android app's property detail.
 *
 * The search endpoints answer no dates at all - read an advert whole and there is nothing to find -
 * while the app's detail carries both `creationDate` and `lastModified`. The portal's own "pubblicato
 * il" is the creation; a listing re-published to the top of the search is what `lastModified` moves
 * for, so the later of the two is what the listing list should order by.
 *
 ...
 * @returns {Promise<ParsedListing>} The same listing, with `publishedAt` when the api answered.
 */
async function fetchDetails(listing) {
  const id = listing?.link?.match(/\/annunci\/(\d+)\//)?.[1];
  if (id == null) return listing;

  try {
    const response = await fetch(`${PROPERTY_DETAIL_URL}${id}`, { headers: { 'User-Agent': USER_AGENT } });
    ...
    // A date the portal would not give is not worth reporting as a run failure; the listing keeps
    // its created_at and the order falls back to what it has always been.
    logger.debug(`Immobiliare.it: no date for ${id} (${error?.message}).`);
  }
  return listing;
}
