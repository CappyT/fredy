 ...
 * running. The endpoint can also be challenged: a 403 switches that run to browser
 * navigation of the homepage, where the DataDome solver can run. API requests
 * then execute inside that browser session.
 *
 ...
 * never reaches the query. Once challenged, subsequent API pages stay in the same
 * browser session rather than repeating requests from the blocked Node transport.
 *
 ...
 *
 * Both transports keep the same delay between pages, with jitter to spread requests out.
 */
...
/**
 * @typedef {object} SearchTransport
 * @property {import('puppeteer-core').Browser} [browser] Browser belonging to this job run.
 * @property {boolean} browserMode Whether an API refusal switched this run to the browser.
 */

/**
 * Open the homepage to handle its challenge, then request the exact API page
 * inside that browser session. An API URL can return a JSON refusal containing a
 * challenge URL rather than an iframe; /search-list/ behaves the same way. The
 * homepage supplies the HTML interstitial the solver needs.
 *
 * @param {URL} target Exact API page URL, including polygons and repeated filters.
 * @param {SearchTransport} transport Run-local transport state.
 * @returns {Promise<any|null>} Parsed response, or null if the challenge remains or JSON is invalid.
 */
async function requestBrowserApiPage(target, transport) {
  let payload = null;
  await puppeteerExtractor(BASE_URL, 'body', {
    ...
    onPage: async (page) => {
      ...
      if (payload == null)
        logger.warn(`Immobiliare.it browser API request answered ${answer.status}; stopping this search.`);
      return payload != null;
    },
  });
  return payload;
...
/**
 * @param {any} payload Search API response from either transport.
 * @returns {{results: any[], maxPages: number}|null} Validated page.
 */
function readApiPayload(payload) {
 ...
 * @param {SearchTransport} transport Run-local browser fallback state.
 * @returns {Promise<{results: any[], maxPages: number}|null>} the page, or null when it did not arrive
 */
async function requestApiPage(endpoint, page, transport) {
  const target = new URL(endpoint);
  target.searchParams.set(PAGE_PARAM, String(page));
  if (transport.browserMode) return readApiPayload(await requestBrowserApiPage(target, transport));
  ...
  if (response.status === 403) {
    ...
    logger.warn('Immobiliare.it API answered 403; switching this search to the browser with DataDome handling.');
    transport.browserMode = true;
    return readApiPayload(await requestBrowserApiPage(target, transport));
  }
  ...
  return readApiPayload(await response.json());
}
 ...
 * @param {number|undefined} known how many pages the search has, when the caller has already been told
 * @param {SearchTransport} transport Run-local browser fallback state.
 * @returns {Promise<any[]>} the raw results of every page read
 */
async function walkApi(endpoint, from, known, transport) {
  const results = [];
  ...
  for (let page = from; page <= Math.min(total, MAX_PAGES); page++) {
    ...
    const answer = await requestApiPage(endpoint, page, transport);
 ...
 */
async function getListingsFromApi(url, transport) {
  return walkApi(convertMapSearchToApi(url), 1, undefined, transport);
}
 ...
 * @returns {Promise<any[]>} the raw results of every page the search has
 */
async function getListingsFromPage(url, browser, transport) {
  logger.debug(`Immobiliare.it: ${url} has to be rendered; the endpoint cannot be asked for it.`);
  const rendered = await puppeteerExtractor(url, 'body', {
    browser,
    name: 'immobiliare',
    // Italian locale when the extractor launches its own browser. A shared
    // job browser retains the locale selected when it was launched.
    acceptLanguage: 'it-IT,it;q=0.9,en;q=0.8',
    // A render may land on the DataDome wall instead of the search; clear it
    // in the page, where the cookie it earns is spent by the reload itself.
    datadome: true,
  });
  ...
  const rest = await walkApi(convertTownSearchToApi(url, first.criteria), 2, first.maxPages, transport);
...
async function getListings(url, browser) {
  const transport = { browser, browserMode: false };
  if (new URL(url).pathname === MAP_SEARCH_PATH) return getListingsFromApi(url, transport);

  const criteria = await translateSearchUrl(url);
  if (criteria != null) {
    const endpoint = convertTownSearchToApi(url, criteria);
    const first = await requestApiPage(endpoint, 1, transport);
    ...
    if (first != null) {
      const rest = first.maxPages > 1 ? await walkApi(endpoint, 2, first.maxPages, transport) : [];
      return [...first.results, ...rest];
    }
    logger.warn('Immobiliare.it refused the search read out of the url, so it is rendered instead.');
  }

  return getListingsFromPage(url, browser, transport);
}
