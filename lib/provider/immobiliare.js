...
import { captchaUrlIn, isSolveable } from '../services/datadome.js';
...
async function requestApiPage(endpoint, page, browser) {
  ...
  for (let attempt = 1; attempt <= READ_ATTEMPTS; attempt++) {
    ...
    if (answer.status !== 200) {
      ...
      //
      // The challenge kind is named separately because the body alone does not show it: `t` sits at
      // the end of a url far longer than any sane log line, so a truncated body hides the one field
      // that says what to do next. `fe` is solvable and means the read can be bought back; `bv`
      // means the exit ip is refused and only a different address helps.
      const challenge = captchaUrlIn(answer.body);
      const kind =
        challenge == null
          ? ''
          : ` (DataDome ${new URL(challenge).searchParams.get('t')}, ` +
            `${isSolveable(challenge) ? 'solvable' : 'not solvable, the exit ip is refused'})`;

      // What the refusal is worth knowing about beyond its body: which page of the walk was
      // refused, how long the browser had been reading by then, and the address it was reading
      // from. A refusal on page one is a search that was never allowed; a refusal deep in a walk
      // that has run past the proxy's exit lifetime is the walk changing address mid-flight. The
      // two want opposite remedies, and the body says nothing about either.
      const ageSeconds = Math.round((Date.now() - (browserFirstSeen.get(browser) ?? Date.now())) / 1000);
      const address = await exitAddress(browser);
      logger.error(
        `Immobiliare.it answered ${answer.status}${kind} on page ${page} of the walk, ` +
          `${ageSeconds}s into the browser, from exit ${address}: ${answer.body.slice(0, 300)}`.trimEnd(),
      );
      return null;
    }
 ...
 */
async function exitAddress(browser) {
  try {
    const answer = await navigateInOwnContext(new URL('https://api.ipify.org/?format=json'), browser);
