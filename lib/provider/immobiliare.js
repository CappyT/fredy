...
import { readToken, tokenForBlock } from '../services/datadome.js';
 ...
 *
 * A residential proxy loses a request now and then: measured in production, a read answered
 * `ERR_INVALID_AUTH_CREDENTIALS` while the same read through the same proxy succeeded a minute
 * later, and a sticky session whose exit node is being re-acquired is the likeliest moment for it.
 * Without this, one lost page ends the whole walk.
 *
 ...
 *
 * @param {URL} target the endpoint url, page parameter already set
 ...
 */
async function requestApiPageInBrowser(target, browser) {
  let lastError = null;
 ...
 *
 * Without a browser the older path stands, token and all: a deployment that has one solved keeps
 * using it, and a caller with no browser to lend is answered whatever the endpoint answers.
 *
...
async function requestApiPage(endpoint, page, browser) {
  ...
  if (browser != null) return requestApiPageInBrowser(target, browser);
  ...
  const token = readToken(target.host);
  if (token != null) headers.Cookie = token;
  ...
  // solved. `tokenForBlock` answers a cookie only when the deployment is configured to solve one,
  // so an unconfigured run keeps the old behaviour and reads the refusal below.
  if (response.status === 403) {
    ...
    if (solved != null) response = await fetch(target, { headers: { ...headers, Cookie: solved } });
  }
