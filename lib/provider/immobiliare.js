 ...
 *
 * A town search ("/vendita-case/milano/") is a Next.js page that server renders its first page of
 * results into `__NEXT_DATA__`, so those are read out of that payload rather than out of the cards.
 * The page is behind DataDome, which the shared browser clears from a residential connection but
 * not from a datacenter one, so a walled render falls through to the service
 ...
 * @param {Record<string, any>} criteria the search the rendered page reports having run
 * @returns {string} the endpoint url to request
 */
export function convertTownSearchToApi(url, criteria) {
  const parsed = new URL(url);
  const endpoint = new URL(LISTINGS_ENDPOINT_PATH, BASE_URL);
  for (const [key, value] of Object.entries(criteria)) endpoint.searchParams.set(key, String(value));
  endpoint.searchParams.set('path', parsed.pathname);
  return endpoint.toString();
}
