...
/** @type {ProviderConfig} */
/**
 * Read the price bounds off a search url.
 *
 * idealista writes its filters as one comma-separated path segment, `con-prezzo_1000,prezzo-min_500`,
 * where the upper bound is the bare `prezzo` and the lower one `prezzo-min` - one dash apart, so
 * the tokens are matched whole rather than by prefix. The same names are what
...
function parsePriceRange(url) {
  const tokens = new URL(url).pathname.split('/').flatMap((segment) => segment.replace(/^con-/, '').split(','));
  const read = (pattern) => tokens.map((token) => token.match(pattern)).find(Boolean)?.[1] ?? null;
  return { min: read(/^prezzo-min_(\d+)$/), max: read(/^prezzo_(\d+)$/) };
}
