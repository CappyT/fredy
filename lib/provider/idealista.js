...
import { probeAdvertActivity, readAdvertModificationDate, searchListings } from '../services/idealista/search.js';
...
/**
 * Whether a stored advert is still online, asked of the api rather than the website.
 *
 * @param {string} link The listing link.
 * @returns {Promise<number>} 1 when online, 0 when gone, -1 when no verdict was obtained.
 */
async function probeActivity(link) {
  const code = link?.match(/\/(\d+)\/?$/)?.[1];
  const portal = portalOf(link);
  if (code == null || portal == null) return checkIfListingIsActive(link);
  return probeAdvertActivity(portal, code);
}
...
const config = {
  ...
  getListings,
  normalize,
  // The date and nothing else: the detail the app opens carries `modificationDate`, and the
  // description a listing shows is the one the search already answered with. Listing a column
  // `fetchDetails` never sets would have the nightly backfill re-read every advert for it forever.
  detailFields: ['publishedAt'],
  fetchDetails,
