...
import { readVerifiedPage, HONEST } from '../services/smg/poison.js';
...
async function getListings(url) {
  ...
  for (let read = 0; read < MAX_PAGES; read++, page++) {
    ...
    if (verdict.verdict !== HONEST) {
      logger.error(
        `Homegate: the page from=${from} of ${url} stayed ${verdict.verdict} after ${attempts} attempts (${verdict.reason}). Its rows are dropped.`,
      );
      break;
    }
