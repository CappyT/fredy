...
import { readThroughGuard } from '../services/http/guardedRead.js';
 ...
 *
 * The app host needs no cookie and is never solved for. `api.homegate.ch` refuses the endpoint
 * without a `datadome` cookie and names the challenge it wants solved. Either host can refuse the
 * exit address the request left from, so `readThroughGuard` owns what a refusal is worth trying:
 * another exit for both hosts, and a solve for the one that is read with a cookie.
 *
...
async function searchVia(endpoint, host, withCookie, body) {
  const answer = await readThroughGuard({
    host,
    name: 'Homegate',
    solve: withCookie,
    send: ({ cookie, dispatcher }) => {
  ...
  if (!answer.ok) {
    ...
    logger.error(
      `Homegate answered ${answer.status} ${answer.statusText} on ${host}: ${answer.body.slice(0, 300)}`.trimEnd(),
    );
    return null;
  }
