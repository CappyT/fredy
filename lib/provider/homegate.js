...
async function getListings(url) {
  ...
  for (let read = 0; read < MAX_PAGES; read++, page++) {
    ...
    const { answer, verdict, attempts } = await readVerifiedPage({ requestPage, body, query, expectNet });
    if (answer == null) break;

    if (verdict.verdict === POISONED) {
      logger.error(
        `Homegate: the page from=${from} of ${url} stayed poisoned after ${attempts} attempts (${verdict.reason}). Its rows are dropped.`,
      );
      break;
    }
