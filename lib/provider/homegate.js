 ...
 *
 * The website sits behind DataDome and refuses a plain client, so this reads the API host the app's
 * own TLS traffic names, `api.re.swissmarketplace.group`. That host answers the search and the
 * location autocomplete with no `datadome` cookie and no challenge. `api.homegate.ch` serves the
 * same API under the portal's name and is kept as the fallback; it refuses without a `datadome`
 * cookie, which `lib/services/datadome.js` solves and keeps.
 *
 * Two endpoints are enough:
 *
 * 1. `GET /geo/locations?lang=…&name=…` turns the location slug of the pasted URL into the geo tag
 *    the search query wants. The answer names each place with a `urlNames` block, and only the entry
 *    whose block spells the whole slug is that URL's place. The lookup is shared with
 *    ImmoScout24.ch, which runs the same SMG code, in `lib/services/smg/geoLocations.js`.
 * 2. `POST /search/listings` runs the search.
 *
 * Every search request carries a non-empty `X-App-Id`. The API answers two value sets for one
 * request - the real listing and a rewritten copy - and a non-empty `X-App-Id` is what makes it
 * answer the real one. The value is not validated, so a constant is enough. Without the header the
 * endpoint serves the rewritten copy for about 70 to 80 percent of the requests, which is why
 * `lib/services/smg/poison.js` still checks every page.
 *
 * `POST /search/listings-by-url`, the endpoint the app uses for a pasted URL, does take a public
 * URL, and it agrees with the translation below: the job's own Chiasso URL answered 200 with
 * `total: 96`, the same total the structured query built from it answers. What it refuses is a URL
 * that is not a result page - `/rent/real-estate-listings/city-zurich` answers 422 "not a SRP uri"
 * - which is where the earlier note that it is unusable came from. Reading through it instead of
 * translating is a decision for the provider as a whole, not one this comment makes.
 */
...
/** The API host the app's own TLS traffic names. It answers without a `datadome` cookie. */
const PRIMARY_HOST = 'api.re.swissmarketplace.group';
/** The same API under the portal's name. It refuses without a `datadome` cookie. */
const FALLBACK_HOST = 'api.homegate.ch';
const PRIMARY_SEARCH = `https://${PRIMARY_HOST}/search/listings`;
const FALLBACK_SEARCH = `https://${FALLBACK_HOST}/search/listings`;
const PRIMARY_LOCATIONS = `https://${PRIMARY_HOST}/geo/locations`;
const FALLBACK_LOCATIONS = `https://${FALLBACK_HOST}/geo/locations`;
...
/**
 * The word a listing URL spells for one offer type, per language: {@link OFFER_TYPES} read back.
 *
 * `{RENT: {en: 'rent', de: 'mieten', …}, BUY: {…}}`. Derived rather than written out, so the
 * vocabulary lives in one place.
 */
const OFFER_TYPE_WORDS = Object.entries(OFFER_TYPES).reduce((words, [word, { offerType, lang }]) => {
  words[offerType] = { ...words[offerType], [lang]: word };
  return words;
}, {});

/**
 * The key the language of the search URL travels under, on a raw entry of the search answer.
 *
 * `normalize` is handed one raw entry and nothing else, so the entry is what carries the language
 * its link has to be spelled in. See {@link getListings}.
 */
const LINK_LANG = '_fredyLinkLang';
 ...
 * @property {boolean} commercial whether the category names a commercial search. The poison
 *   detector needs it: a commercial-only rent search carries no `prices.rent.net` in either value
 *   set, so the net-price signal cannot judge it.
...
function parseSearchUrl(url) {
  ...
  return {
    ...
    commercial: COMMERCIAL_CATEGORIES.has(category),
...
/**
 * Ask one host's search endpoint for one page.
 *
 * The app host needs no cookie. `api.homegate.ch` refuses the endpoint without a `datadome` cookie
 * and names the challenge it wants solved; `tokenForBlock` answers a cookie only when the
 * deployment is configured to solve one, so an unconfigured run reads the refusal as before.
 *
 * @param {string} endpoint the search endpoint of one host
 * @param {string} host the host, for the token store and the solve
 * @param {boolean} withCookie whether this host is read with a `datadome` cookie
 * @param {Object} body the request body
 * @returns {Promise<any|null>} the page, or null when it did not arrive
 */
async function searchVia(endpoint, host, withCookie, body) {
  const token = withCookie ? readToken(host) : null;
  ...
  // successful answer, whose body is read as json below. The refusal body is read at most once.
  let refusalText = null;

  if (response.status === 403 && withCookie) {
    refusalText = await response.text().catch(() => '');
    if (isDataDomeBlock(response.status, refusalText)) {
  ...
  if (!response.ok) {
    ...
    const refused = refusalText ?? (await response.text().catch(() => ''));
    logger.error(
      `Homegate answered ${response.status} ${response.statusText} on ${host}: ${refused.slice(0, 300)}`.trimEnd(),
    );
    return null;
  }
...
/**
 * Ask the search for one page, from the app host, and fall back to the portal host when it does not
 * answer. Only the fallback can pay for a solve.
 *
 * @param {Object} body The request body.
 * @returns {Promise<any|null>} The page, or null when it did not arrive.
 */
async function requestPage(body) {
  try {
    const primary = await searchVia(PRIMARY_SEARCH, PRIMARY_HOST, false, body);
    if (primary != null) return primary;
  } catch (error) {
    logger.debug(`Homegate: ${PRIMARY_HOST} could not be reached (${error?.message ?? error}).`);
  }
  logger.debug('Homegate: the app host did not answer the search; falling back to api.homegate.ch.');
  try {
    return await searchVia(FALLBACK_SEARCH, FALLBACK_HOST, true, body);
  } catch (error) {
    logger.error(`Homegate: neither host answered the search (${error?.message ?? error}).`);
    return null;
  }
}
...
async function getListings(url) {
  ...
  if (search.locationSlug != null) {
    const geoTag =
      (await resolveGeoLocationId({
        endpoint: PRIMARY_LOCATIONS,
        slug: search.locationSlug,
        lang: search.lang,
        headers: requestHeaders(),
      })) ??
      (await resolveGeoLocationId({
        endpoint: FALLBACK_LOCATIONS,
        slug: search.locationSlug,
        lang: search.lang,
        headers: requestHeaders(),
      }));
  ...
  const expectNet = search.offerType === 'RENT' && !search.commercial;
  const listings = [];
  ...
  for (let read = 0; read < MAX_PAGES; read++, page++) {
    ...
    if (verdict.verdict === POISONED) {
      logger.error(
        `Homegate: the page from=${from} of ${url} stayed poisoned after ${attempts} attempts (${verdict.reason}). Its rows are dropped and the walk stops there.`,
      );
      break;
    }
    ...
    if (verdict.verdict === UNKNOWN && !saidUnverified) {
      saidUnverified = true;
      logger.warn(
        `Homegate: the page from=${from} of ${url} could not be verified (${verdict.reason}). Its rows are kept.`,
      );
    }

    const found = Array.isArray(answer.results) ? answer.results : [];
    // The link has to name the offer type in the language of the pasted URL, which only the search
    // knows. Non-enumerable, so the row stays the answer the API sent.
    for (const entry of found) {
      if (entry != null) Object.defineProperty(entry, LINK_LANG, { value: search.lang, configurable: true });
    }
    listings.push(...found);
 ...
 *
 * The three names are read off the live response: 96 listings of a Chiasso search carried
 * `street`, `postalCode`, `locality`, `region`, `geoCoordinates` and `geoTags`, and none carried
 * `city`, `zip` or `houseNumber`. `street` already holds the house number ("Corso San Gottardo
 * 96"), so no number is appended.
 *
 * `streetAddition` is left out: the one value the search answered was ", Chiasso", which is the
 * locality again and would reach the geocoder twice.
 *
 * A shape that carries none of them answers null, which costs the listing its address rather than
 * inventing one.
 *
...
function buildAddress(address) {
  ...
  const town = [address?.postalCode, address?.locality].map(clean).filter(Boolean).join(' ');
 ...
 * the offer type word in lower case, in the language of the search URL, then the id. Verified on
 * the live portal: `/affittare/4003459485` resolves, `/RENT/4003459485` answers 404, and the API
 * answers `offerType` in upper case.
 *
 * @param {any} listing
 * @param {string} [lang] the language of the search URL. English when no search is known.
 * @returns {string|null}
 */
function listingLink(listing, lang = 'en') {
  if (listing?.id == null || String(listing.id).length === 0) return null;
  const offerType = String(listing.offerType ?? 'RENT').toUpperCase();
  const words = OFFER_TYPE_WORDS[offerType];
  const word = words?.[lang] ?? words?.en ?? offerType.toLowerCase();
  return `${BASE_URL}${word}/${listing.id}`;
}
...
function normalize(o) {
  ...
  return {
    ...
    link: listingLink(listing, o?.[LINK_LANG]),
    price,
