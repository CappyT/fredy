...
/**
 * A browser's user agent. Tecnocasa serves the same document either way, but a request without one
 * is the obvious thing to rate limit first.
 */
const USER_AGENT =
 ...
 * carries a version tecnocasa bumps on its own schedule - the advert page is on `estate-show-v1`
 ...
 * that went in.
 *
 * @param {string|null|undefined} html the raw html of a tecnocasa page
 ...
 */
function readComponentData(html, attribute, tagPrefix) {
  if (!html) return null;
  ...
  try {
    ...
    logger.error(`Could not parse tecnocasa ${attribute}.`, error?.message || error);
    return null;
  }
}
...
async function getListings(url) {
  ...
  if (!response.ok) {
    logger.error(`Error fetching data from tecnocasa: ${response.status} ${response.statusText}`);
    return [];
  }

  const estates = readComponentData(await response.text(), ':estates');
  if (!Array.isArray(estates)) {
    logger.error('Tecnocasa returned a page without adverts. The search URL may be wrong.');
    return [];
  }
  return estates;
}
...
/**
 * Turn a price as tecnocasa writes it into a number.
 *
 ...
 *
 * Tecnocasa stores it as markup. The paragraph and line breaks have to survive as newlines: a
...
async function fetchDetails(listing) {
  try {
    ...
    if (!response.ok) {
      logger.warn(`Could not fetch tecnocasa advert '${listing.id}': ${response.status} ${response.statusText}`);
      return listing;
    }
    ...
    logger.warn(`Could not fetch tecnocasa advert '${listing.id}'.`, error?.message || error);
    return listing;
  }
}
...
const config = {
  ...
  crawlContainer: null,
  crawlFields: {},
  // Tecnocasa offers the sort in its own interface but its server ignores the parameter it sets:
  // every search comes back in the same order whichever way it is asked for, with the agencies'
  // promoted adverts first. There is nothing to send, so a search is worth keeping narrow enough
  // that a new advert lands on the first page.
