...
import { localPublicationDate, publicationDate, wallClockInZone } from '../utils/publicationDate.js';
...
/** The path of an advert's own page, `/immobili/<id>/`. */
const ADVERT_PATH = /^\/immobili\/(\d+)\/?$/;
...
async function getListingsFromPage(url, browser) {
  ...
  for (let page = 1; page <= MAX_PAGES; page++) {
    ...
    const html = await puppeteerExtractor(pageUrl(url, page), 'body', {
      browser,
      name: 'casa',
      ...
      // Casa.it is behind the same wall: it walls the third page of a walk
      // that turns them as fast as the browser renders, so every page gets
      // the wall cleared off it when one shows.
      datadome: true,
    });
...
function normalizeAdvert(o) {
  ...
  return {
    ...
    // has no date until `fetchDetails` reads one off the advert's page.
...
/**
 * Read the day an advert was last updated off its own page.
 *
 * Only a search read off the website needs it: the api stamps every advert, and a listing that
 * already has a date is returned without a request. The page's schema.org listing names the day as
 * `lastReviewed`.
 *
 * @param {ParsedListing} listing the listing built from the search
 * @param {import('puppeteer').Browser} browser the shared browser of the current job run
 * @returns {Promise<ParsedListing>} the listing with the recovered date, or the untouched one
 */
async function fetchDetails(listing, browser) {
  if (publicationDate(listing.publishedAt) != null) return listing;
  try {
    const id = ADVERT_PATH.exec(parseCasaUrl(listing.link)?.pathname ?? '')?.[1];
    if (id == null) return listing;
    const link = `${BASE_URL}/immobili/${id}/`;
    const html = await puppeteerExtractor(link, 'body', { browser, name: 'casa_details' });
    const detail = readNextData(html)?.props?.pageProps?.pdp;
    if (String(detail?.id) !== id) return listing;
    const graph = detail?.schema?.data?.['@graph'];
    if (!Array.isArray(graph)) return listing;
    const advert = graph.find(
      (node) => node?.['@id'] === link && [node?.['@type']].flat().includes('RealEstateListing'),
    );
    const publishedAt = localPublicationDate(advert?.lastReviewed, CASA_ZONE);
    return publishedAt == null ? listing : { ...listing, publishedAt };
  } catch (error) {
    logger.warn(`Could not fetch Casa.it details for listing '${listing?.id}'.`, error?.message || error);
    return listing;
  }
}
...
const config = {
  ...
  getListings,
  normalize,
  // No `detailFetchDelayMs`: the pipeline would also wait before the listings the api already
  // dated, which `fetchDetails` returns without a request.
  fetchDetails,
