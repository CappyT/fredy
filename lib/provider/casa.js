...
function normalizeAdvert(o) {
  ...
  const place = geo.street || geo.district_name || geo.block_name;
  const address = [place, geo.city].filter((part) => typeof part === 'string' && part.trim().length > 0);
  ...
  return {
    ...
    address: address.length === 0 ? null : address.join(', '),
    latitude: geo.lat,
    longitude: geo.lon,
