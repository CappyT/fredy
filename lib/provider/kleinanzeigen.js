...
import { localPublicationDate, publicationDate, relativePublicationDate } from '../utils/publicationDate.js';
...
function extractDetailFromHtml(html) {
  ...
  return {
    detailAddress,
    detailDescription,
    publishedAt: localPublicationDate(
      $('#viewad-extra-info .icon-calendar-gray-simple').next('span').text().trim(),
      'Europe/Berlin',
    ),
...
async function enrichListingFromDetails(listing, browser) {
  ...
  try {
    ...
    const { detailAddress, detailDescription, detailSize, detailRooms, detailBuildYear, publishedAt } =
    ...
    return {
      ...
      description,
      publishedAt: publishedAt ?? listing.publishedAt,
...
/**
 * Read search cards and recover their dates from the page's embedded Astro props.
 * @param {string} url The search URL.
 * @param {import('puppeteer-core').Browser} [browser] The job's shared browser.
 * @returns {Promise<Object[]>} Raw search listings with optional publication dates.
 */
async function getListings(url, browser) {
  const html = await puppeteerExtractor(url, config.waitForSelector, { browser, name: 'kleinanzeigen' });
  const fetchedAt = Date.now();
  const listings = parse(config.crawlContainer, config.crawlFields, html, url) ?? [];
  if (!listings.length) return listings;
  const $ = cheerio.load(html);
  const dates = new Map();
  // The cards omit dates in the rendered HTML, but the impression tracker retains them by advert ID.
  for (const element of $('astro-island[props]').toArray()) {
    try {
      const rows = JSON.parse($(element).attr('props')).resultAds?.[1];
      if (!Array.isArray(rows)) continue;
      for (const row of rows) {
        const advert = row?.[1]?.organicAdPreview?.[1];
        const id = advert?.id?.[1];
        const date = advert?.sortingDate?.[1];
        if (id == null) continue;
        const timestamp =
          localPublicationDate(date, 'Europe/Berlin') ?? relativePublicationDate(date, 'Europe/Berlin', fetchedAt);
        if (timestamp != null) dates.set(String(id), timestamp);
      }
    } catch {
      continue;
    }
  }
  return listings.map((listing) => ({ ...listing, publishedAt: dates.get(String(listing.id)) }));
}
