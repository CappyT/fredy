 ...
 * @property {Array<'description'|'publishedAt'>} [detailFields] Which stored columns this provider's `fetchDetails` is actually able to fill. Read by the nightly detail backfill, which sweeps a row only when a column *this* provider can answer is empty: the sweep used to ask by row rather than by provider, so idealista adverts were re-read every fortnight for a description idealista's detail read never returns. Absent means the provider is not swept at all - which is the right answer for the ones whose `fetchDetails` enriches something else entirely, or needs a browser the sweep does not have. It is a static claim about the code in `fetchDetails`, so it belongs on the template beside the pacing numbers; a field listed here that `fetchDetails` never sets costs a request per row per retry window, forever.
