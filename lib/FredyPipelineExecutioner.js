...
import {
  ...
  attachTravelTimes,
  deleteListingsById,
  getKnownListingHashesForJob,
  getKnownListingsByLinkForJob,
  storeListings,
  storeListingImage,
  updateListingDistances,
} from './services/storage/listingsStorage.js';
import { notifyPriceChanges, getThresholdPercent } from './services/listings/priceHistoryService.js';
import { recordScrapedPriceChange } from './services/listings/scrapedPriceChange.js';
import { fetchListingImage } from './services/listings/imageFetcher.js';
...
import { getCurrencyForListing } from './services/providers/providerCurrency.js';
...
/** Photographs downloaded in parallel at scrape time. The CDNs serve browsers doing worse. */
 ...
 * 9) Geocode, and tag each listing with the currency its price is written in
...
class FredyPipelineExecutioner {
  ...
  execute() {
    return (
      Promise.resolve(urlModifier(this._providerConfig.url, this._providerConfig.sortByDateParam))
        ...
        .then(this._assignCurrency.bind(this))
        ...
        // After both soft-deleting filters, for the same reason the benchmark is: a photograph is
        // a download and a blob per listing, and paying for one on a row that is being tombstoned
        // in the next step buys nothing the user will ever see.
  ...
  /**
   * Tag each new listing with the currency its price is written in.
   *
   * Right before the insert, so the row is stored with it and everything after - the benchmark, the
   * notification - reads it off the listing. Per listing for the same reason geocoding is: the
   * currency follows the country, and a provider covering several markets narrows that per advert.
   *
   * @param {ParsedListing[]} newListings
   * @returns {Promise<ParsedListing[]>} The same listings, each with `currency` set.
   */
  async _assignCurrency(newListings) {
    for (const listing of newListings) {
      listing.currency = await getCurrencyForListing(this._providerId, listing);
    }
    return newListings;
  }
  ...
  /**
   * Determine which listings are new by comparing their IDs against the hashes already stored
   * for this job, whichever provider stored them.
   *
   * The check is job-wide because the table's uniqueness already is: `idx_listings_job_hash`
   * spans providers, so a second provider's insert of an advert the job already stores updates
   * nothing and leaves no history of its own. The tecnocasa group is the case in point -
   * tecnocasa.it and tecnorete.it advertise the same records under one group-wide estate id - and
   * a per-provider check there made the second brand re-notify the first brand's adverts on every
   * run.
   *
   * The hash is built over the portal's advert id *and its price*, so an advert that dropped its
   * price hashes differently and would pass this check as new - stored a second time under its
   * link while the row already holding the link stayed active beside it. A listing that is new by
   * hash therefore gets a second identity check by link: an advert the job already knows is the
   * same flat with a new price, and it goes through the price-change lane - one history row, the
   * threshold, the price-change notification - instead of through this one.
   *
   * @param {ParsedListing[]} listings Listings to evaluate for novelty.
   * @returns {Promise<ParsedListing[]>} New listings not seen before.
  ...
  async _findNew(listings) {
    ...
    const knownHashes = new Set(getKnownListingHashesForJob(this._jobKey) || []);

    const candidates = listings.filter((o) => !knownHashes.has(o.id));
    const knownByLink = new Map(
      getKnownListingsByLinkForJob(
        this._jobKey,
        candidates.map((o) => o.link),
      ).map((row) => [row.link, row]),
    );

    const newListings = [];
    const priceChanges = [];
    // Read once for the batch, not once per candidate: it is the same settings row every time.
    const thresholdPercent = await getThresholdPercent();
    for (const listing of candidates) {
      const stored = listing.link == null ? null : knownByLink.get(listing.link);
      if (stored == null) {
        newListings.push(listing);
        continue;
      }
      // The candidate's id *is* the hash the portal now answers to - the row keeps its own primary
      // key and takes the new name, so the next run's hash lane recognises it and this lane is not
      // asked to rediscover the same change. The stored row brings only its id, job and old price, so
      // the reading supplies what the notification shows. Price change notifications carry no photo.
      const shown = { title: listing.title, address: listing.address, size: listing.size, rooms: listing.rooms };
      const change = recordScrapedPriceChange({ ...stored, ...shown }, listing.price, listing.id, {
        source: 'scrape',
        thresholdPercent,
      });
      if (change != null) priceChanges.push(change);
    }
  ...
  /**
   * Keep the photograph each stored listing was found with.
   *
   * The url a portal hands out is a rental - idealista signs its cloudfront links for about a
   * day, and every portal takes photographs down when the advert changes - so the bytes are
   * downloaded once, at scrape time, and the listing serves its own copy from then on. The row id
   * `storeListings` has just propagated onto each listing is what the bytes are keyed by.
   *
   * A download that fails costs the listing nothing but its photograph for now: the startup
   * backfill picks it up later, which is also why this step must never let one rejection reach
   * the run.
   *
   * It runs after the similarity and area filters rather than straight after the insert. Both of
   * those soft-delete what they reject, and a listing on its way out is one whose photograph
   * nobody will ever open - downloading and storing a blob for it is a request at the portal and a
   * row in the database spent on a tombstone.
   *
   * @param {ParsedListing[]} newListings The listings just stored, each carrying its row id.
   * @returns {Promise<ParsedListing[]>} The same listings, unchanged.
   */
  async _storeImages(newListings) {
    const withImage = newListings.filter((listing) => listing?.image != null && listing?.id != null);
    if (withImage.length === 0) return newListings;

    await mapLimit(withImage, IMAGE_FETCH_CONCURRENCY, async (listing) => {
      try {
        const image = await fetchListingImage(listing.image);
        if (image != null) storeListingImage(listing.id, image.mimeType, image.bytes);
      } catch (error) {
        logger.debug(`Could not keep the image of listing '${listing.id}' (${error?.message}).`);
      }
    });
    return newListings;
  }
