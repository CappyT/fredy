...
class FredyPipelineExecutioner {
  ...
  async _fetchDetails(newListings) {
    ...
    for (const [index, listing] of listingsToEnrich.entries()) {
      ...
      // requests as fast as the network allows is the behaviour that earns a block - which is how
      // the rest of the batch was left without its date. The provider knows its own host's
      // tolerance; the first request needs no breath before it.
  ...
  async _findNew(listings) {
    ...
    for (const listing of candidates) {
      ...
      // asked to rediscover the same change.
      const change = recordPriceChange(stored, listing.price, {
        source: 'scrape',
        thresholdPercent,
        newHash: listing.id,
      });
