...
export function localPublicationDate(value, timeZone) {
  ...
  try {
    ...
    const inZone = (timestamp) => {
      const fields = Object.fromEntries(formatter.formatToParts(timestamp).map(({ type, value }) => [type, value]));
      return ['year', 'month', 'day', 'hour', 'minute', 'second'].map((key) => Number(fields[key]));
    };
    const offsets = [-86400000, 0, 86400000].map((delta) => {
      const [y, m, d, h, min, sec] = inZone(utc + delta);
      return Date.UTC(y, m - 1, d, h, min, sec) - (utc + delta);
    });
    const candidates = [...new Set(offsets)]
      .map((offset) => utc - offset)
      .filter((timestamp) => inZone(timestamp).every((value, index) => value === parts[index]));
...
export function relativePublicationDate(value, timeZone, referenceTime = Date.now()) {
  ...
  try {
    ...
    const fields = Object.fromEntries(
      formatter.formatToParts(referenceTime - elapsed).map(({ type, value }) => [type, value]),
    );
