 ...
 * with a working proxy in the wrong country instead of an error. Everything the caller does not know
 * about (a city, a state, `streaming`) is carried through untouched, so a password written by hand
 * survives a rewrite.
 *
 * Two callers rewrite it. The settings page writes the country, the sticky switch and the lifetime;
 * the scraper writes a session id IPRoyal has not seen, which is how a refused read leaves from
 * another exit node (`newIsolatedPage` in `../extractor/puppeteerExtractor.js`).
 *
 * The browser may not import out of `lib/`, so `ui/src/services/proxy/iproyal.js` is a copy of
 * everything below plus the helper only the form needs. `test/ui/iproyalInSync.test.js` fails when
 * the two read or write a password differently.
 */
