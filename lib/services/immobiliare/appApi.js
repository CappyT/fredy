...
import { describeChallenge } from '../datadome.js';
import { readThroughGuard } from '../http/guardedRead.js';
 ...
 * datacenter exit answered 403 with the `fe` challenge. A refusal is therefore asked again from
 * another exit before a `fe` challenge is offered to the solver, which is what `readThroughGuard`
 * does; `lib/services/datadome.js` owns the solve and its cost cap. A read nothing could rescue
 * falls back to the browser, which renders the website instead.
 *
...
export async function fetchAppPage(params, start) {
  // Which page of the walk was refused, as the website path already names it: a refusal on page one
  // is a search that was never allowed, one deep in a walk is the guard closing mid-read.
  ...
  let answer;
  try {
    answer = await readThroughGuard({
      host: APP_HOSTNAME,
      name: 'Immobiliare.it: the app api',
      page,
  ...
  if (!answer.ok) {
    logger.warn(
      `Immobiliare.it: the app api answered ${answer.status}${describeChallenge(answer.body)} on page ${page} ` +
        'of the walk, rendering instead.',
    );
    return null;
  }
