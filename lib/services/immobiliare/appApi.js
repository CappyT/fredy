...
export async function fetchAppPage(params, start) {
  ...
  if (answer.status === 403) {
    ...
    if (!isDataDomeBlock(answer.status, blocked)) {
      logger.warn(
        `Immobiliare.it: the app api answered ${answer.status} on page ${page} of the walk, rendering instead.`,
      );
      return null;
    }
    ...
    if (solved == null) {
      logger.warn(
        `Immobiliare.it: the app api answered 403${challengeKind(blocked)} on page ${page} of the walk, ` +
          'rendering instead.',
      );
      return null;
    }
