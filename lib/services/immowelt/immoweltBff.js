 ...
 * fires the first request half-warmed. A warm-up that answers 403 has met the wall instead, and
 * the solver clears it before the cookie is waited for - the cookie the reload carries is the one
 * this whole warm-up exists to earn. Not getting the cookie is not treated as fatal - not every
...
async function acquireWarmPage(browser) {
  ...
  const pending = (async () => {
    ...
    try {
      const response = await page.goto(`${IMMOWELT_ORIGIN}/`, {
        waitUntil: 'domcontentloaded',
        timeout: WARMUP_NAVIGATION_TIMEOUT,
      });
      // The warm-up earns the cookie every later request rides on, so a wall in
      // front of even the home page has to be cleared first: the cookie the
      // XHRs need is the one the cleared reload carries. Anything else is left
      // to the cookie wait below, which knows how to wait for it.
      if (response?.status?.() === 403) {
        await solveCaptcha(page, { response });
      }
      await page
