...
export const trackJobPriceObservation = async (job, providers = []) => {
  try {
    ...
    const { medianPriceOfListings, medianPricePerSqm, currency } = getListingsKpisForJobIds([job.id]);
    // Nothing measurable yet: no listing of this job carries a price per square metre.
    if (medianPricePerSqm == null) return;
    // The receiving side reads every figure as euros. A job whose listings are mostly in francs has
    // nothing it could report without passing one currency off as the other.
