 ...
 * - `bassa-efficienza` and the "only auctions" tick, whose url names were never seen on a page,
 *   where the two energy names below and `aste_no` come off a live search url
 * - the letting terms
 *
...
/**
 * One website box that the api has no single name for: "Appartamenti" covers flats, penthouses and
 * two-level flats together, and the api honours one shape of home per search. A url naming it is
 * run once per shape - alongside the houses' own search where the url names those too - and the
 * answers are merged.
 */
const APARTMENTS_TOKEN = 'appartamenti';

/** The searches "Appartamenti" stands for, each one shape the api does read. */
const APARTMENT_SHAPES = /** @type {Array<[string, string]>} */ ([
  ['flat', '1'],
  ['penthouse', '1'],
  ['duplex', '1'],
]);
 ...
 *   more than one only when the url names several building conditions, or the "Appartamenti" box
 *   the api serves one shape of home at a time - or null when a filter has no counterpart and the
 *   search therefore cannot be asked of the api at all
 */
export function readFilters(segment) {
  ...
  let apartments = false;
  for (const token of segment.replace(/^con-/, '').split(',')) {
    if (token === APARTMENTS_TOKEN) {
      apartments = true;
      continue;
    }
  ...
  let shapes = [[]];
  if (apartments) {
    // "Appartamenti" already covers the narrower flat boxes, so a switch naming one merges into it.
    for (const [name] of APARTMENT_SHAPES) params.delete(name);
    shapes = APARTMENT_SHAPES.map((shape) => [shape]);

    const houses = params.get('subTypology');
    if (houses != null) {
      shapes.push([['subTypology', houses.join(',')]]);
      params.delete('subTypology');
    }
  }
  ...
  return shapes.flatMap((shape) => splits.map((split) => [...common, ...shape, ...split]));
}
