...
export async function fetchSearchHtml(url, browser) {
  ...
  try {
    ...
    // A 403 is DataDome's interstitial: most often it clears itself and
    // reloads, occasionally it asks for a drag. The solver waits out the one
    // and performs the other, so the wait below either sees a page that was
    // always fine or one the wall has already given up on.
