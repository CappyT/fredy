...
import { readToken, tokenForUrl, cookieParts, SOLVE_USER_AGENT } from '../datadome.js';
...
export async function fetchSearchHtml(url, browser) {
  ...
  try {
    // DataDome guards these pages. When the deployment solves its challenges, the token earned for
    // this host is set before the navigation, so the page arrives instead of the challenge. The
    // browser has to use the very user agent the token was solved with, which is also the only kind
    // capsolver accepts. `tokenForUrl` answers null when no solver is configured, and nothing about
    // the navigation changes then.
    const host = new URL(url).host;
    const token = readToken(host) ?? (await tokenForUrl(url, SOLVE_USER_AGENT).catch(() => null));
    if (token != null) {
      const { name, value } = cookieParts(token);
      await page.setUserAgent(SOLVE_USER_AGENT);
      await page.setCookie({ name, value, url: `https://${host}/` });
    }
