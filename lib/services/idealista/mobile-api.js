 ...
 */
async function fire(path, query, body, retryOnUnauthorised, method = 'POST') {
  ...
  let response;
  try {
    response = await fetch(`${BASE_URL}${path}?${queryString(fullQuery)}`, options);
  } catch (error) {
  ...
  if (response.status === 401 && retryOnUnauthorised) {
    ...
    forgetToken();
    return fire(path, query, body, false, method);
  }
 ...
 *
 * @param {string} path
 * @param {{query?: Array<[string, unknown]>, body?: Array<[string, unknown]>, method?: 'POST'|'GET', retryOnUnauthorised?: boolean}} [options]
 * @returns {Promise<any>} the parsed response
 * @throws {Error} when the api answers anything but 2xx, and while a throttle silence lasts
 */
export async function call(path, { query = [], body = [], method = 'POST', retryOnUnauthorised = true } = {}) {
  return enqueue(() => fire(path, query, body, retryOnUnauthorised, method));
}
