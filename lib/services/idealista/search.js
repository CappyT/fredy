...
import { parseSearchUrl } from './deeplink.js';
...
export async function searchListings(webUrl) {
  let parsed = await parseSearchUrl(webUrl);
  let search = parsed?.search ?? null;
  let area = parsed?.area ?? null;

  if (search == null || area == null) {
    search = translateSearchUrl(webUrl);
    area = search == null ? null : await areaOf(search);
  }
...
/** The one advert's detail, which is where the api keeps the dates the search does not carry. */
const DETAIL_PATH = '/api/3/it/detail/';
 ...
 */
export async function readAdvertModificationDate(propertyCode) {
  const detail = await call(`${DETAIL_PATH}${propertyCode}`, {
    method: 'GET',
    query: [
      ['language', 'it'],
      ['quality', 'high'],
