 ...
 *
 * A search is asked for by place where the url names one, and by outline where it names areas in
 * the website's own shorthand - see `./zones.js`. Where the url names several building conditions
 * the api has to be asked once per condition, because it takes one at a time, and the answers are
 * merged.
 */

import { call, MAX_ITEMS_PER_PAGE, SEARCH_PATH } from './mobile-api.js';
import { resolveLocationId } from './locations.js';
import { locationsOf, outlineOf } from './zones.js';
 ...
 * runs of a job. The rest is the margin for a job that was stopped for a while, and for the first
 * run of a new one, which has the whole search to catch up on.
 */
const MAX_PAGES = 3;
