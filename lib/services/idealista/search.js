...
/**
 * Whether an advert is still online, asked of its detail.
 *
 * The website's advert pages sit behind DataDome, so probing them from a server gets no verdict,
 * and the alive checker deactivates a listing after enough of those. The detail answers a server:
 * `state` reads `active` for an advert that is online, and a code it does not know is a 404. Any
 * other state or failure is no verdict.
 *
 * @param {Portal} portal The site the advert lives on.
 * @param {string} propertyCode The code that names the advert on the website.
 * @returns {Promise<number>} 1 when online, 0 when gone, -1 when no verdict was obtained.
 */
export async function probeAdvertActivity(portal, propertyCode) {
  try {
    const detail = await readAdvertDetail(portal, propertyCode);
    return detail?.state === 'active' ? 1 : -1;
  } catch (error) {
    return /** @type {any} */ (error)?.httpStatus === 404 ? 0 : -1;
  }
}
 ...
 * @param {Portal} portal The site the advert lives on.
 * @param {string} propertyCode The code that names the advert on the website.
 * @returns {Promise<any>} The detail payload.
 * @throws {Error} when the api answers anything but 2xx (see `call`)
 */
function readAdvertDetail(portal, propertyCode) {
