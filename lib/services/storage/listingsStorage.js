...
export const getListingsToEnrichConnectivity = ({ limit, maxAgeDays, now = Date.now() }) => {
  ...
  return SqliteConnection.query(
    `SELECT id, latitude, longitude, provider, address
     FROM listings
     WHERE is_active = 1
       AND manually_deleted = 0
...
/**
 * Listings of the named providers whose description never arrived.
 *
 * The backfill sweep's work list. Only providers that know where to ask for the text are named by
 * the caller - a row of any other provider would be swept forever, because nobody would ever fill
 * it, so the query refuses to hand one out. Active listings only: a gone advert has no description
 * left to read anywhere, and retention is about to delete the row anyway. Soft-deleted rows stay
 * in, on the other hand - a human can bring one back at any time, and a listing returning from the
 * hidden view without its text would read as broken rather than as once-hidden.
 *
 * @param {string[]} providers - Provider ids that have a description enricher.
 * @returns {Array<{id: string, link: string, provider: string}>}
 */
export const getListingsMissingDescription = (providers) => {
...
/**
 * Listings of the named providers whose publication date never arrived.
 *
 * The backfill sweep's work list, built the same way the description sweep's is: only providers
 * that know where to ask for the date are named by the caller, and only active rows are handed
 * out, since a gone advert has no date left to read anywhere. The rows this list exists for are
 * the ones a run stored while its detail reads were being refused - a burst of new listings can
 * earn a block partway through the batch, and the listings past that point were stored with the
 * column left empty.
 *
 * @param {string[]} providers - Provider ids that have a publication-date enricher.
 * @returns {Array<{id: string, link: string, provider: string}>}
 */
export const getListingsMissingPublishedAt = (providers) => {
  const ids = Array.isArray(providers) ? providers.filter((p) => typeof p === 'string' && p.length > 0) : [];
  if (ids.length === 0) return [];
  return SqliteConnection.query(
    `SELECT id, link, provider
