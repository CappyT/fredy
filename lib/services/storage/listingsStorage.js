...
/**
 * Every listing hash already stored for a job, whichever provider stored it.
 *
  ...
  SqliteConnection.withTransaction((db) => {
    ...
    const stmt = db.prepare(
      `INSERT INTO listings (id, hash, provider, job_id, price, size, rooms, build_year, energy_class, title,
                             image_url, description, address, link, created_at, is_active, latitude, longitude, published_at)
       VALUES (@id, @hash, @provider, @job_id, @price, @size, @rooms, @build_year, @energy_class, @title, @image_url,
               @description, @address, @link, @created_at, 1, @latitude, @longitude, @published_at)
...
export const getListingImage = (listingId) => {
  ...
  return SqliteConnection.query(`SELECT mime_type, bytes FROM listing_images WHERE listing_id = ?`, [listingId])[0] ?? null;
};
 ...
 * their rows must not be moved by a scrape. Inactive rows stay in - an advert that went away and
 * returned is the same advert, and the alive-checker, not the scrape, owns bringing the row back.
 *
