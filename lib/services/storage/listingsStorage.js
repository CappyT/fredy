...
/**
 * Every listing hash a job still recognises, whichever provider stored it.
 *
 * The uniqueness the table enforces is on `(job_id, hash)` across providers - a decision the
 * tecnocasa group makes necessary, since tecnocasa.it and tecnorete.it advertise the same records
 * under the same group-wide estate id, so both brands hash identically and only one row can exist.
 * The novelty check has to read the same set or a second brand re-finds the first brand's adverts
 * on every run: its own per-provider history stays empty (its inserts collide with the stored rows
 * and update nothing), while the similarity cache never learned them because the user had already
 * hidden the copies it would have matched against.
 *
 * Which rows count as recognised is the same question {@link getKnownListingsByLinkForJob} answers,
 * and the two have to answer it identically - they are the two lanes of one novelty check, and a
 * row one of them remembers while the other has forgotten it falls between them:
 *
 * - A row the alive-checker declared gone is forgotten. The advert behind it is being answered by
 *   the portal again, which is a repost, and a repost is stored and brought back to life rather
 *   than dropped. While the hash lane still knew such a row, a repost at the unchanged price was
 *   filtered out here and never reached the link lane that would have stored it, and a price that
 *   later returned to the dead row's price was swallowed the same way.
 * - A row the user hid is still remembered, exactly as before. Hiding a listing says "I am done
 *   with this advert", and forgetting its hash would re-find it, re-notify it, and collide with the
 *   hidden row on insert. This is also what the similarity and area filters lean on: their
 *   tombstones are `manually_deleted` rows, and they exist precisely so a listing is not
 *   rediscovered on every run.
 *
 * @param {string} jobId
 * @returns {string[]} Known hashes for the job.
 */
export const getKnownListingHashesForJob = (jobId) => {
  return SqliteConnection.query(
    `SELECT hash
     FROM listings
     WHERE job_id = @jobId
       AND (manually_deleted = 1 OR is_active IS NULL OR is_active = 1)`,
    { jobId },
  ).map((r) => r.hash);
};
 ...
 *
 * @param {string[]} jobIds
 * @returns {{ numberOfActiveListings: number, medianPriceOfListings: number, medianPricePerSqm: Object|null, currency: string }}
 *   `currency` is the one both medians are quoted in.
 */
export const getListingsKpisForJobIds = (jobIds = []) => {
  if (!Array.isArray(jobIds) || jobIds.length === 0) {
    return { numberOfActiveListings: 0, medianPriceOfListings: 0, medianPricePerSqm: null, currency: DEFAULT_CURRENCY };
  }

  const placeholders = jobIds.map(() => '?').join(',');
  const scope = `FROM listings WHERE job_id IN (${placeholders}) AND manually_deleted = 0`;
  ...
  // A median over francs and euros together is a figure in neither, so both medians are taken in
  // one currency: the one most priced listings are in, the default one on a tie. It travels with
  // the numbers so the dashboard can say which currency it is quoting.
  const dominant = SqliteConnection.query(
    `SELECT COALESCE(currency, '${DEFAULT_CURRENCY}') AS pricedCurrency, COUNT(price) AS pricedCount
     ${scope}
     GROUP BY 1
     ORDER BY pricedCount DESC, pricedCurrency = '${DEFAULT_CURRENCY}' DESC
     LIMIT 1`,
    jobIds,
  )[0];
  const currency = Number(dominant?.pricedCount ?? 0) > 0 ? dominant.pricedCurrency : DEFAULT_CURRENCY;
  const from = `${scope} AND COALESCE(currency, '${DEFAULT_CURRENCY}') = ?`;

  const activeCount = Number(counts?.activeCount ?? 0);
  const pricedCount = Number(dominant?.pricedCount ?? 0);
  ...
  return {
    ...
    medianPricePerSqm: medianPricePerSqmForJobIds(jobIds, currency),
    currency,
  };
};
 ...
 *
 * @param {string[]} jobIds
 * @param {string} [currency] Only listings priced in this currency, the same one the median price
 *   is taken in. A row with no currency yet reads as the default one.
 ...
 *   migration.
 */
function medianPricePerSqmForJobIds(jobIds, currency = DEFAULT_CURRENCY) {
  const placeholders = jobIds.map(() => '?').join(',');
  const from = `FROM listings l
                ...
                WHERE l.job_id IN (${placeholders})
                  ...
                  AND COALESCE(l.currency, '${DEFAULT_CURRENCY}') = ?
                  AND j.deal_type = ?`;

  let best = null;
  for (const dealType of [DEAL_TYPES.RENT, DEAL_TYPES.BUY]) {
    const params = [...jobIds, currency, dealType];
...
export const getListingsDueForPriceCheck = ({
  ...
  return SqliteConnection.query(
    `SELECT id, link, provider, price, job_id, title, address, size, rooms, currency
     FROM listings
     WHERE (is_active IS NULL OR is_active = 1)
       AND manually_deleted = 0
...
/**
 * The hash a row is allowed to take, falling back to the one it already carries.
 *
 * `(job_id, hash)` is unique, so a rename onto a name a sibling row of the same job already holds
 * is a constraint violation - which aborts the whole run, not just the rename. It is reachable:
 * an advert the alive-checker declared gone is stored again as a fresh row when the portal reposts
 * it at a different price, and the corpse keeps the hash of the old price until retention removes
 * it, so a return to that price asks the live row to take a name the dead one still holds. Leaving
 * the row on its old hash there costs one redundant trip through the link lane per run; throwing
 * costs the run.
 *
 * @type {string}
 */
const HASH_IF_FREE_SQL = `CASE
           WHEN @newHash IS NULL THEN hash
           WHEN EXISTS (SELECT 1
                        FROM listings other
                        WHERE other.job_id = listings.job_id
                          AND other.hash = @newHash
                          AND other.id <> listings.id) THEN hash
           ELSE @newHash
         END`;
 ...
 *
 * The hash moves with the price when the caller knows the new one. Most providers hash the price
 * in, so the row that kept its old hash after a change was the advert under a name the portal no
 * longer answers with: the scrape recognised it by link every single run and re-recorded the same
 * change, a return to a previously seen price was swallowed by the hash lane before the link lane
 * ever saw it, and hiding the row let the advert come back as a brand-new listing. The price probe
 * reads a price off a page and knows no hash at all, which is why this is a parameter rather than
 * something computed here: passing nothing leaves the hash exactly as it was.
 *
 * @param {string} listingId
 * @param {number} newPrice
 * @param {number} [changedAt=Date.now()]
 * @param {string|null} [newHash=null] The hash the advert now answers to, when the caller read one.
 * @returns {{changes: number}|undefined}
 */
  ...
  return SqliteConnection.execute(
    `UPDATE listings
     SET previous_price = price,
         ...
         hash = ${HASH_IF_FREE_SQL}
     WHERE id = @listingId`,
    { listingId, newPrice: Math.round(newPrice), changedAt, newHash: nullOrEmpty(newHash) ? null : newHash },
  );
};

/**
 * Move a listing onto the name the portal now answers with, without touching its price.
 *
 * The price probe reads a detail page and knows no hash, so a row it moved keeps the hash the
 * scrape gave it - which no longer matches the price the row now carries. The next scrape finds
 * that advert under a hash the job does not recognise, hands it to the link lane, and is told the
 * price has not moved; without this the rename that the price lane would have done never happened,
 * so the same advert dropped through the same lane on every run and a return to its earlier price
 * was swallowed by the hash lane. A rename is not a price reading: no history row, no notification.
 *
 * @param {string} listingId
...
export const renameListingHash = (listingId, newHash) => {
  if (!listingId || nullOrEmpty(newHash)) return undefined;
  return SqliteConnection.execute(
    `UPDATE listings
     SET hash = ${HASH_IF_FREE_SQL}
     WHERE id = @listingId`,
    { listingId, newHash },
  );
};
...
export const storeListings = (jobId, providerId, listings) => {
  ...
  SqliteConnection.withTransaction((db) => {
    ...
    //
    // The conflicting row is brought back to life, because reaching this statement at all means the
    // portal is answering with the advert again: the novelty check hands over a listing whose hash
    // it no longer recognises, and the only stored rows it has forgotten are the ones the
    // alive-checker declared gone. Without it a repost was announced to the user and then left
    // invisible on a dead row - and, since the row stayed dead, announced again on every run after
    // that. A row the user hid is exempt, hence the `WHERE`: hiding an advert is a decision no
    // scrape may undo, and the novelty check never offers such a row here anyway.
    const stmt = db.prepare(
      `INSERT INTO listings (id, hash, provider, job_id, price, size, rooms, build_year, energy_class, title,
                             ...
                             published_at, price_per_sqm, currency)
       ...
       ON CONFLICT(job_id, hash) DO UPDATE SET published_at = COALESCE(@published_at, listings.published_at),
                                               ...
                                               inactive_since = NULL,
                                               active_check_failures = 0
         WHERE listings.manually_deleted = 0
       RETURNING id`,
    );
    ...
    for (const item of listings) {
      const params = {
        ...
        // Set by the pipeline from the provider's country. Left NULL when a caller stores listings
        // without one, which every query reads as the default currency.
...
export const applyMarketBenchmark = (jobId, listings) => {
  ...
  for (const listing of listings) {
    // Only listings in the same currency are comparable. A listing that carries none is measured
    // against everything, as it was before currencies existed.
    const benchmark = computeBenchmark(listing, dealType, runQuery, { currency: listing.currency ?? null });
    if (benchmark == null) {
      continue;
    }
...
export const queryListings = ({
  ...
  if (affordabilityBand != null) {
    // The profile's thresholds are euros, and the finance model behind them is a German one, so a
    // listing priced in any other currency has no verdict and never belongs to a band.
    whereParts.push(`(COALESCE(l.currency, '${DEFAULT_CURRENCY}') = '${DEFAULT_CURRENCY}')`);
    const arms = [];
 ...
 *
 * @returns {Object[]} Array of {id, latitude, longitude, provider, address, link}.
 */
export const getListingsToEnrichConnectivity = ({ limit, maxAgeDays, now = Date.now() }) => {
  ...
  return SqliteConnection.query(
    `SELECT id, latitude, longitude, provider, address, link
     FROM listings
     WHERE is_active = 1
       AND manually_deleted = 0
...
/**
 * How long a row that was asked and gave nothing is left alone, in days.
 *
 * Long enough that a portal having a bad night is not a reason to give up on an advert, short
 * enough that the whole back catalogue is not re-read every week. The sweep can override it.
 * @type {number}
 */
export const DETAIL_BACKFILL_RETRY_DAYS = 14;

/**
 * How many rows one detail sweep is allowed to take, mirroring the image sweep's own ceiling.
 *
 * The list used to be unbounded, which on an instance with a large back catalogue meant every tick
 * materialised the whole `listings` join before a single request went out - and the startup sweep,
 * which the image sweep waits behind, ran for as long as it took to walk all of it. A run takes a
 * batch and the nightly cron takes the next; the rows are paced at roughly half a second each
 * either way, so a bound is the difference between a long sweep and an endless one.
 * @type {number}
 */
export const DETAIL_BACKFILL_BATCH_SIZE = 250;

/**
 * Which stored column each declared detail field fills, as the predicate for "still empty".
 *
 * The names are the ones a provider lists in `detailFields`; a name that is not here is ignored
 * rather than guessed at, so a typo narrows a sweep instead of widening it.
 * @type {Record<string, string>}
 */
const DETAIL_FIELD_MISSING_SQL = {
  description: "(l.description IS NULL OR l.description = '')",
  publishedAt: 'l.published_at IS NULL',
};

/**
 * Listings whose provider can still fill a column their scrape left empty.
 *
 * One work list for one detail read. Both columns come off the same page - the description and
 * the portal's own date are two fields of the answer immobiliare's app api gives - so two sweeps
 * asking separately meant two requests per row against a host the code itself paces for, and the
 * second one learned nothing the first could not have told it.
 *
 * What each provider can answer is the caller's declaration, not an assumption: a row is offered
 * only when a column *that provider's* `fetchDetails` actually sets is missing. Asking by row
 * rather than by provider was re-reading idealista adverts every fortnight for a description
 * idealista's detail read never returns - it sets the publication date and nothing else - so those
 * rows came back on every sweep for as long as they existed. A provider that declares nothing is
 * simply not swept.
 *
 * Active rows only - a gone advert has no description left to read anywhere, and retention is about
 * to delete it - and never a row the user hid, which is a listing they have said they are done
 * with. Those two, with the link check, are also the partial index this query is served by
 * (`idx_listings_detail_backfill`), which is what lets the walk stop at `limit` rows instead of
 * sorting the whole table into a temporary b-tree first.
 *
 * `detail_backfill_at` is what keeps the list finite. It records the attempt, not the answer, so
 * an advert whose page has gone, one whose portal states no date, and one whose host refused all
 * fall off the list for `retryDays` instead of being re-read every night forever. Never-attempted
 * rows sort first - NULL leads an ascending sort - so a fresh listing is never starved by a
 * backlog, and among the rest the longest-rested comes first.
 *
 * The job owner comes along because the sweep is a detail fetch like the pipeline's, and the same
 * `provider_details` opt-in decides whether this user's rows may be read at all - a question SQL is
 * left out of on purpose. The setting is JSON in a key/value table, written in two shapes across
 * the migrations that produced it, and a `json_each` join would have to guess which one a row is
 * in and throws outright on anything that is not valid JSON. The sweep filters in JS instead and
 * marks what it rejects; see `listing-detail-cron.js` for what that costs.
 *
 * @param {Record<string, string[]>} providerFields - Provider id to the detail fields its `fetchDetails` fills.
 * @param {Object} [options]
 * @param {number} [options.now=Date.now()] - Injectable for tests.
 * @param {number} [options.retryDays=DETAIL_BACKFILL_RETRY_DAYS] - How long an attempted row rests.
 * @param {number} [options.limit=DETAIL_BACKFILL_BATCH_SIZE] - Ceiling on how many rows one sweep takes.
 * @returns {Array<{id: string, link: string, provider: string, user_id: string|null, needs_description: number, needs_published_at: number}>}
 */
export const getListingsMissingDetails = (
  providerFields,
  { now = Date.now(), retryDays = DETAIL_BACKFILL_RETRY_DAYS, limit = DETAIL_BACKFILL_BATCH_SIZE } = {},
) => {
  const clauses = [];
  const params = [];
  for (const [provider, fields] of Object.entries(providerFields ?? {})) {
    if (typeof provider !== 'string' || provider.length === 0) continue;
    const missing = (Array.isArray(fields) ? fields : [])
      .map((field) => DETAIL_FIELD_MISSING_SQL[field])
      .filter((predicate) => predicate != null);
    if (missing.length === 0) continue;
    clauses.push(`(l.provider = ? AND (${[...new Set(missing)].join(' OR ')}))`);
    params.push(provider);
  }
  if (clauses.length === 0) return [];

  const restDays = Number.isFinite(retryDays) && retryDays >= 0 ? retryDays : DETAIL_BACKFILL_RETRY_DAYS;
  const attemptedBefore = now - restDays * 24 * 60 * 60 * 1000;
  const cap = Number.isFinite(limit) && limit > 0 ? Math.floor(limit) : DETAIL_BACKFILL_BATCH_SIZE;
  return SqliteConnection.query(
    `SELECT l.id,
            l.link,
            l.provider,
            j.user_id AS user_id,
            (l.description IS NULL OR l.description = '') AS needs_description,
            (l.published_at IS NULL) AS needs_published_at
     FROM listings l
     LEFT JOIN jobs j ON j.id = l.job_id
     WHERE l.is_active = 1
       AND l.manually_deleted = 0
       AND l.link IS NOT NULL
       AND (l.detail_backfill_at IS NULL OR l.detail_backfill_at <= ?)
       AND (${clauses.join(' OR ')})
     ORDER BY l.detail_backfill_at, l.created_at DESC
     LIMIT ${cap}`,
    [attemptedBefore, ...params],
  );
};

/**
 * Record that a listing's detail page was asked for, whatever came back.
 *
 * Written on every attempt, including the ones that answered nothing: the point of the marker is
 * to stop a row nobody can fill from being re-read on every sweep, and only a row that was tried
 * knows it was tried.
 *
 * @param {string} id - The listing's row id.
 * @param {number} [attemptedAt=Date.now()]
 * @returns {void}
 */
export const markDetailBackfillAttempt = (id, attemptedAt = Date.now()) => {
  if (!id || !Number.isFinite(attemptedAt)) return;
  SqliteConnection.execute(`UPDATE listings SET detail_backfill_at = @attemptedAt WHERE id = @id`, { id, attemptedAt });
};

/**
 * Store the description a listing was found without when it was scraped.
 *
 * @param {string} id - The listing's row id.
 * @param {string} description - The description text.
 * @returns {void}
 */
export const updateListingDescription = (id, description) => {
  if (!id || nullOrEmpty(description)) return;
  SqliteConnection.execute(`UPDATE listings SET description = @description WHERE id = @id`, { id, description });
};

/**
 * Store the publication date a listing was stored without.
 *
 * @param {string} id - The listing's row id.
 * @param {number} publishedAt - The date the portal states, as epoch milliseconds.
 * @returns {void}
 */
export const updateListingPublishedAt = (id, publishedAt) => {
  if (!id || !Number.isFinite(publishedAt)) return;
  SqliteConnection.execute(`UPDATE listings SET published_at = @publishedAt WHERE id = @id`, { id, publishedAt });
};

/**
 * Store the fresh image url a listing's stale one was replaced by.
 *
 * @param {string} id - The listing's row id.
 * @param {string} imageUrl - The url as the portal just signed it.
 * @returns {void}
 */
export const updateListingImage = (id, imageUrl) => {
  if (!id || nullOrEmpty(imageUrl)) return;
  SqliteConnection.execute(`UPDATE listings SET image_url = @imageUrl WHERE id = @id`, { id, imageUrl });
};

/**
 * Keep the bytes of a listing's photograph.
 *
 * The portal's url is a rental - idealista signs its links for about a day, and the others take
 * photographs down when the advert changes - so the scrape downloads the image once and these
 * bytes are what the listing serves from then on. A renewed photograph replaces the stored one
 * rather than accumulating: the latest picture is the one the portal is showing.
 *
 * @param {string} listingId - The listing's row id.
 * @param {string} mimeType - The content type the portal served, e.g. "image/webp".
 * @param {Buffer} bytes - The image bytes.
 * @returns {{changes: number}|undefined}
 */
export const storeListingImage = (listingId, mimeType, bytes) => {
  if (!listingId || nullOrEmpty(mimeType) || !Buffer.isBuffer(bytes) || bytes.length === 0) return undefined;
  return SqliteConnection.execute(
    `INSERT INTO listing_images (listing_id, mime_type, bytes, fetched_at)
     VALUES (@listingId, @mimeType, @bytes, @fetchedAt)
     ON CONFLICT(listing_id) DO UPDATE SET
       mime_type = @mimeType, bytes = @bytes, fetched_at = @fetchedAt`,
    { listingId, mimeType, bytes, fetchedAt: Date.now() },
  );
};

/**
 * The stored bytes of a listing's photograph, or null when none were kept.
 *
 * @param {string} listingId - The listing's row id.
 * @returns {{mime_type: string, bytes: Buffer}|null}
 */
export const getListingImage = (listingId) => {
  if (!listingId) return null;
  return (
    SqliteConnection.query(`SELECT mime_type, bytes FROM listing_images WHERE listing_id = ?`, [listingId])[0] ?? null
  );
};

/**
 * Active listings of the named providers whose photograph was never kept.
 *
 * The backfill's work list, one row behind the scrape: the pipeline keeps the bytes of every
 * listing as it is stored, so what this list holds is the backlog an instance upgraded with - and
 * the odd scrape whose download failed the moment it ran. Only listings the portal showed an
 * image for are handed out; a listing without one has nothing to fetch anywhere, and naming a
 * provider the caller cannot refresh keeps its rows off the list rather than retried forever.
 *
 * Rows the user hid are left out for the same reason the detail list leaves them out: a download
 * and a blob for a listing somebody has said they are done with is a request at the portal spent
 * on a photograph nobody will open.
 *
 * @param {string[]} providers - Provider ids that may be fetched for.
 * @param {number} [limit] - Cap the list, so a large backlog is worked through in batches.
 * @returns {Array<{id: string, link: string, image_url: string, provider: string}>}
 */
export const getListingsMissingStoredImage = (providers, limit) => {
  const ids = Array.isArray(providers) ? providers.filter((p) => typeof p === 'string' && p.length > 0) : [];
  if (ids.length === 0) return [];
  const cap = Number.isFinite(limit) && limit > 0 ? ` LIMIT ${Math.floor(limit)}` : '';
  return SqliteConnection.query(
    `SELECT l.id, l.link, l.image_url, l.provider
     FROM listings l
     WHERE l.is_active = 1
       AND l.manually_deleted = 0
       AND l.link IS NOT NULL
       ...
       AND l.provider IN (${ids.map(() => '?').join(',')})
     ORDER BY l.created_at DESC${cap}`,
    ids,
  );
};

/**
 * The stored listings of one job that already carry one of the given links.
 *
 * The hash a listing is deduplicated by is built over the portal's advert id *and its price* -
 * upstream's shape, kept so an unhashed id can never collide - which makes a price change read as
 * a brand-new advert. The identity that survives a price change is the link, so the pipeline asks
 * for the stored rows behind the links of the listings it was about to call new. One row per link
 * comes back: the newest, which is the one whose price the portal is currently answering with.
 *
 * Listings the user hid are left out on purpose: they are not coming back as notifications, and
 * their rows must not be moved by a scrape. So are the ones the alive-checker has declared gone.
 * Nothing brings a row back to life on its own - `reactivateListings` is only ever reached from
 * the manual route - so absorbing a repost into a dead row buried it: the price landed on a row
 * the listing view does not show, the advert was announced as a price drop rather than as a find,
 * and it was never stored again. An advert the portal is answering with once more is a listing
 * again, and it is stored as one.
 *
 * @param {string} jobId - The job whose listings are searched.
 * @param {string[]} links - The links of the listings about to be treated as new.
 * @returns {Array<{id: string, link: string, price: number|null, provider: string, job_id: string}>}
 */
export const getKnownListingsByLinkForJob = (jobId, links) => {
  ...
  const found = [];
  for (let offset = 0; offset < cleaned.length; offset += ID_CHUNK_SIZE) {
    const chunk = cleaned.slice(offset, offset + ID_CHUNK_SIZE);
    const rows = SqliteConnection.query(
      `SELECT id, link, price, provider, job_id, created_at, currency
       FROM listings
       WHERE job_id = ?
         AND manually_deleted = 0
         AND (is_active IS NULL OR is_active = 1)
         AND link IN (${chunk.map(() => '?').join(',')})
  ...
  // The query hands back every row that ever carried the link - the duplicates a price change
  // used to leave behind among them. The newest is the row to move.
  const newestPerLink = new Map();
  for (const row of found) {
    if (!newestPerLink.has(row.link)) newestPerLink.set(row.link, row);
  }
