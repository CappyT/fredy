...
import { isIproyalProxy, randomSessionId, readIproyalOptions, writeIproyalOptions } from '../proxy/iproyal.js';
...
/**
 * Whether a request can be sent from another exit node than the one in force.
 *
 * Only IPRoyal spells its exit wishes in the password, so only an IPRoyal proxy can be steered. Any
 * other proxy, and no proxy at all, leaves from the one address it has, and a read refused for that
 * address stays refused however often it is asked again.
 *
 * @returns {boolean}
 */
export function canRotateExit() {
  return isIproyalProxy(appliedProxyUrl);
}

/**
 * A dispatcher that leaves from an exit node IPRoyal has not used yet.
 *
 * A session id IPRoyal has not seen starts a new session on a new exit node, with no lifetime to
 * wait out, which is how a refused read leaves from another address. A password with no session
 * segment already rotates on every request, so the installed dispatcher is a new exit by itself and
 * the answer is null - the same null a caller gets when there is nothing to steer.
 *
 * The caller closes what it gets, once the request it was built for has been read.
 *
 * @returns {import('undici').Dispatcher|null} The dispatcher to send one request through, or null
 *   when the installed one already leaves from a new address.
 */
export function rotatedExitDispatcher() {
  const options = readIproyalOptions(appliedProxyUrl);
  if (options == null || !options.sticky) return null;

  return createOutboundDispatcher(writeIproyalOptions(appliedProxyUrl, { ...options, sessionId: randomSessionId() }));
}
