...
export function initJobExecutionService({ providers, intervalMs }) {
  ...
  async function executeJob(job) {
    ...
    try {
      ...
      const liveSettings = await getSettings();
      const proxyUrl = resolveProxyUrl(liveSettings);
      ...
      for (const prov of jobProviders) {
        try {
          ...
          if (!browser) {
            // The native input humanizer fights precisely-timed input: a drag
            // the solver walks in two seconds arrives on the page as twenty
            // seconds of tremoring interpolation. The job flow only navigates
            // and reads, so its own input is the solver's - which wants the
            // raw channel.
            //
            // Bot guards also score a headless Chrome lower than a windowed
            // one even when its fingerprint is patched - a wall a solved
            // captcha had cleared re-arms itself against a headless session.
            // The image ships a virtual display for exactly this, so wherever
            // a display exists the browser gets a window in it.
            const headless = !(process.env.IS_DOCKER && process.env.DISPLAY);
            browser = await puppeteerExtractor.launchBrowser(providerConfig.url, {
              humanize: false,
