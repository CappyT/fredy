...
import { sourceForCountries, sourcesForCountries, SOURCES, SOURCE_IDS } from './sources.js';
 ...
 * @param {string|undefined} [address] The listing's address, as the portal printed it. The
 *   registers that answer by point never read it; the Italian checker cannot be asked by point and
 ...
 */
export async function getConnectivity(lat, lng, countries, address) {
  ...
  // Which of a country's registers answers is the operator's choice, expressed as the switches
  // they left on: the first one still switched on. Nothing left on means nothing to ask, which is
  // a different silence from "no register covers this country" and reads the same from here.
  const source = sourceForCountries(countries, (id) => isSourceEnabled(settings, id));
  if (source == null) {
    return null;
  }

  // Only a source that reads the address may have the address in its key: for a register asked by
  // point, two spellings of the same place are one lookup and must stay one.
  const key =
    source.keyedBy === 'address' ? cacheKey(source.id, lat, lng, address ?? '') : cacheKey(source.id, lat, lng);
  const connectivity = await cached(key, () => source.fetch(lat, lng, address));
