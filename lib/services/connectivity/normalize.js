 ...
 * @property {string[]} [networks] The wholesale networks that reach the address, best first. Only
 *   a source that names them carries this; the registers that answer per cell do not know.
...
/**
 * The technologies the Italian checker names, keyed to Fredy's fixed-line vocabulary.
 *
 * The checker splits fibre into two codes - one for the network builder's offer, one for the
 * retail one - which are the same copper-or-glass question for a listing. VDSL and EVDSL both end
 * at the cabinet, and FWA is the wireless answer an address gets where nothing is buried.
 * @type {Record<string, string>}
 */
export const IT_TECHNOLOGIES = {
  FTTH: 'ftthb',
  FTTHNB: 'ftthb',
  EVDSL: 'fttc',
  VDSL: 'fttc',
...
/**
 * Turns what the Italian checker answers for one address into the shape the rest of Fredy reads.
 *
 * The checker names commercial offers rather than a register cell - "TIM, FTTC, 102 Mbit/s" - so
 * the fastest offer per technology is the honest reading of what the address can get. There is no
 * cell and therefore no household share, and no mobile answer: the fixed-line registers of the
 * other countries carry one because their grid layers do, and this checker does not ask that
 * question.
 *
 * An empty answer is a verdict - the address is unserved - and arrives as connectivity with no
 * speed and no fibre, which is what the card renders as "nothing on record".
 *
 * @param {Array<{technology: string, download_speed: number, upload_speed?: number, carrier?: string}>|null|undefined} offers
 * @returns {Connectivity}
 */
export function normalizeItalian(offers) {
  /** @type {Record<string, TechnologyCoverage>} */
  const technologies = {};
  let maxDownMbit = null;

  for (const offer of Array.isArray(offers) ? offers : []) {
    const key = IT_TECHNOLOGIES[String(offer?.technology ?? '').toUpperCase()];
    const down = Number(offer?.download_speed);
    if (key == null || !Number.isFinite(down) || down <= 0) continue;

    const coverage = technologies[key] ?? { maxDownMbit: null, sharePercent: null };
    if (coverage.maxDownMbit == null || down > coverage.maxDownMbit) {
      coverage.maxDownMbit = down;
    }
    technologies[key] = coverage;

    if (maxDownMbit == null || down > maxDownMbit) {
      maxDownMbit = down;
    }
  }
  ...
  return {
    maxDownMbit,
    sharePercent: null,
    fiber: technologies.ftthb != null,
    technologies,
    mobile: null,
    source: 'it-navigabene',
  };
}

/**
 * The wholesale networks fibermap names, keyed by the prefix of its own technology codes.
 *
 * Its keys are the technology and the network in one - `FTTH_FC` is FiberCop's fibre, `FTTH_OF_AB`
 * Open Fiber's in the areas it built commercially, `FWA_4G_OPNET` and `FWA_EOLO` the two wireless
 * carriers - so the technology is the part in front of the first underscore, and the rest is who
 * carries it.
 *
 * `ADSL` is deliberately absent. It is copper straight from the exchange, has no place in Fredy's
 * fixed-line vocabulary, and giving it one would put a chip on every card in three languages for
 * the least interesting answer there is. It still counts towards the headline speed, exactly the
 * way the German register's "all technologies" ladder counts everything it knows and its
 * per-technology ladders name only three.
 * @type {Record<string, string>}
 */
export const IT_NETWORK_TECHNOLOGIES = {
  FTTH: 'ftthb',
  FTTB: 'ftthb',
  FTTC: 'fttc',
  VDSL: 'fttc',
...
/**
 * What fibermap's sellability codes mean when they mean "not here".
 *
 * Every technology in the answer carries its own vocabulary - FiberCop's fibre is `Attivo`,
 * `Pianificato`, `Sospeso`, `Saturo` or `NO`, a wireless carrier's is just `Coperto`, a dedicated
 * line's is a class letter - and the site's own catalogue reads all of them back to one of two
 * things: an address the network reaches, or one it does not. These two are the ones that mean it
 * does not. `Saturo` and `Sospeso` stay in: the network is at the door and the question is whether
 * a port is free today, which is between the buyer and the operator rather than a property of the
 * flat.
 * @type {string[]}
 */
const IT_UNREACHABLE = ['NO', 'PIANIFICATO'];

/**
 * How many networks are carried alongside the verdict.
 *
 * Five is already every network measured at one address. The cap is here so a future answer that
 * lists thirty resellers cannot quietly grow every listing row in the database.
 * @type {number}
 */
const IT_MAX_NETWORKS = 8;

/**
 * Whether one of fibermap's entries describes a network that reaches the building.
 *
 * @param {Record<string, unknown>} entry
 * @returns {boolean}
 */
function reachable(entry) {
  if (Number(entry?.stato) !== 1) return false;
  return !IT_UNREACHABLE.includes(
    String(entry?.vendibile ?? '')
      .trim()
      .toUpperCase(),
  );
}
...
/**
 * Turns what fibermap answers for one building into the shape the rest of Fredy reads.
 *
 * Only the `shared` half is read. The `dedicated` half is business access - Fibra Dedicata, GEA,
 * FTTO, BEA - which is a symmetric line quoted per site and built to order, and it answers 1000
 * Mbit/s at almost every address in a town. Counting it would tell somebody looking at a flat that
 * they can have a gigabit when what they can have is a quotation.
 *
 * The verdict is the fastest network that reaches the door, and the networks that reach it travel
 * alongside as names - "FiberCop NGA FTTH", "Open Fiber FTTH Aree Nere A&B" - because which
 * wholesale network is at the building is the part that decides who somebody can actually buy
 * from, and it is the one thing this source knows that the other italian one does not.
 *
 * There is no cell and therefore no household share, and no mobile answer: the aggregator does not
 * ask that question.
 *
 * @param {{shared?: Record<string, Record<string, unknown>>|unknown[]}|null|undefined} network The
 *   `network` half of a coverage answer.
 * @returns {Connectivity|null} `null` when there is no network half at all, which is a malformed
 *   answer rather than an unserved address. An address every network answers `NO` for is a verdict
 *   and comes back as connectivity with no speed and no fibre, which the card renders as "nothing
 *   on record".
 */
export function normalizeFibermap(network) {
  if (network == null || typeof network !== 'object') {
    return null;
  }

  const shared = network.shared;
  const entries = shared != null && typeof shared === 'object' && !Array.isArray(shared) ? Object.entries(shared) : [];

  /** @type {Record<string, TechnologyCoverage>} */
  const technologies = {};
  /** @type {Array<{down: number, name: string}>} */
  const reached = [];
  let maxDownMbit = null;

  for (const [code, entry] of entries) {
    if (!reachable(entry)) continue;
    const down = Number(entry?.speed_dl);
    if (!Number.isFinite(down) || down <= 0) continue;

    reached.push({ down, name: String(entry?.copertura ?? code) });
    if (maxDownMbit == null || down > maxDownMbit) {
      maxDownMbit = down;
    }

    const key = IT_NETWORK_TECHNOLOGIES[String(code).split('_')[0].toUpperCase()];
  ...
  const networks = [...new Set(reached.sort((one, other) => other.down - one.down).map((entry) => entry.name))].slice(
    0,
    IT_MAX_NETWORKS,
  );
