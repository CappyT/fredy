...
import { sourceForCountries, sourcesForCountries, SOURCE_IDS } from './sources.js';
 ...
 * Enrich one listing.
 *
 * @param {{id: string, latitude: number, longitude: number, provider: string, address?: string, link?: string}} listing
...
async function enrichListing(listing, settings, now) {
  ...
  // whole point here: idealista covers Spain, Italy and Portugal, Italy is the only one of the
  // three with a register, and asking the provider would send every Madrid and Lisboa listing to
  // the Italian checkers - which read the address as Italian, spend one of fibermap's five requests
  // an hour on it, and stamp the row "nothing here" for the next six months.
  ...
  if (sourcesForCountries(countries).length === 0) {
    updateListingConnectivity(listing.id, null, toColumns(null), now);
    return 'empty';
  }

  // Whichever of the country's registers the operator has left switched on, and nothing at all
  // when they have switched off every one of them. A register the operator has switched off is not
  // a register that answered "nothing here": asked before the lookup rather than after, because
  // the lookup returns the same empty answer either way, and stamping on it would put every
  // listing in that country out of reach for the whole retry interval - including after the
  // operator switches the register back on.
  const source = sourceForCountries(countries, (id) => isSourceEnabled(settings, id));
  if (source == null) {
    return 'skipped';
  }
  ...
  const result = await getConnectivity(listing.latitude, listing.longitude, countries, listing.address);
