 ...
 *
 * @param {{where: any[], filters: Record<string, any>, sort?: string[], page?: number, size?: number}} search
 * @returns {Promise<{total: number, results: any[]}|null>} the page, or null when the api refused it
 */
export async function search({ where, filters, sort = [], page = 1, size = PAGE_SIZE }) {
  const body = {
    site: SITE,
    page,
    size,
