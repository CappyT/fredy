 ...
 * in the agency's timezone; read as UTC like {@link casa.js} reads the same format off casa.it, an
 * hour's drift does not matter to a date that answers "how old is this advert".
 *
...
export function createNetworkConfig(network) {
  return {
    ...
    // Both: the estate payload the detail read fetches carries the advert's text and the date it
    // was last published, and `fetchDetails` above puts each of them on the listing.
