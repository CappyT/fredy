...
/**
 * Run the alive check now, then on its schedule.
 *
 * The startup pass is what notices the adverts that went away while Fredy was down; the column the
 * checker writes is also what keeps the pass bounded, so a restart re-probes only what is due
