 ...
 * honest when it has no signal to stand on. Such a page is `unknown`, and the caller treats an
 * unknown page like a poisoned one - it is logged and its rows are skipped, never stored.
 *
 * @module smg/poison
 */
...
/**
 * Read one page, repeat the same request while the answer is not honest, and give back the honest
...
export async function readVerifiedPage({ requestPage, body, query, expectNet = false }) {
  ...
  while (verdict.verdict !== HONEST && attempts < MAX_VERIFY_ATTEMPTS) {
