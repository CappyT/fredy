 ...
 * @param {Record<string, string>} [headers] the app headers to ask with, when the caller has them
 * @returns {Promise<any|null>} the parsed answer, or null when it did not arrive
 */
async function askLocations(endpoint, name, lang, headers) {
 ...
 *
 * @param {{endpoint: string, slug: string, lang: string, headers?: Record<string, string>}} location
 *   the portal's endpoint, the slug the URL wrote, the URL's language and, when the caller has them,
 *   the app headers to ask with
 * @returns {Promise<string|null>} the geo id, or null when the slug resolved nothing
 */
export async function resolveGeoLocationId({ endpoint, slug, lang, headers }) {
  for (const name of candidateNames(slug)) {
    const answer = await askLocations(endpoint, name, lang, headers);
