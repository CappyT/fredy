...
/**
 * The proxy credentials each browser was launched with.
 *
 ...
 *
 * @type {WeakMap<import('puppeteer-core').Browser, {username: string, password: string}>}
 */
 ...
 * @returns {void}
 */
function rememberProxyAuth(browser, proxyUrl) {
  if (!proxyUrl) return;
  try {
    const { username, password } = new URL(proxyUrl);
    if (!username) return;
    proxyAuthByBrowser.set(browser, {
 ...
 * authentication is what {@link proxyAuthByBrowser} exists for.
 *
...
export async function newIsolatedPage(browser) {
  ...
  const auth = proxyAuthByBrowser.get(browser);
  if (auth != null) await page.authenticate(auth);
  return { page, context };
}
...
export async function launchBrowser(url, options) {
  ...
  rememberProxyAuth(browser, options?.proxyUrl);
  return browser;
}
