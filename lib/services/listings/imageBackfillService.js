...
async function keepImage(listing) {
  ...
  if (image == null && listing.provider === 'idealista') {
    ...
    if (code != null) {
      const advert = await readAdvert(code, listing.link);
 ...
 * will simply wait. It runs *after* the detail backfill rather than beside it: both walk the same
 * back catalogue at the same handful of portals, and starting them together doubles the traffic an
 * instance opens with.
 *
...
export async function runImageBackfill() {
  ...
  sweepRunning = true;
  try {
    const listings = getListingsMissingStoredImage([
      'idealista',
      'immobiliare',
      'tecnocasa',
      'tecnorete',
