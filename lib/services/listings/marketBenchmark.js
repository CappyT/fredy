...
/**
 * {@link COMPARABLES_SQL}, restricted to listings priced in one currency.
 *
 * Near a border the bounding box reaches across it - Basel, Geneva and Lugano sit within fifteen
 * kilometres of euro adverts - and a median over francs and euros together describes neither market.
 * A row with no currency yet reads as the default one, which is what it was taken to be before the
 * column existed.
 *
 * A second query rather than a condition in the first, because migration 41 prepares the first one
 * on a fresh database long before migration 900 adds the column it would name.
 *
 * @type {string}
 */
export const COMPARABLES_IN_CURRENCY_SQL = COMPARABLES_SQL.replace(
  'AND l.price > 0',
  `AND l.price > 0\n    AND COALESCE(l.currency, '${DEFAULT_CURRENCY}') = @currency`,
);
 ...
 * @param {string|null} [options.currency] Compare only against listings priced in this currency,
 *   through {@link COMPARABLES_IN_CURRENCY_SQL}. Absent means every currency, which is all a caller
 *   running before the `currency` column exists can ask for.
 ...
 */
export function computeBenchmark(listing, dealType, runQuery, { currency = null } = {}) {
  ...
  const rows = runQuery(currency == null ? COMPARABLES_SQL : COMPARABLES_IN_CURRENCY_SQL, {
