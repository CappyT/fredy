...
import { getDirectDispatcher } from '../../services/http/outboundProxy.js';
...
export const send = async ({ serviceName, newListings, notificationConfig, jobKey, baseUrl }) => {
  ...
  let fetchOptions = {
    ...
    // The endpoint belongs to whoever runs Fredy, not to a portal, so it is reached directly even
    // when a proxy is configured: the proxy is metered, and an internal endpoint is often only
...
export const sendPriceChange = async ({ serviceName, priceChanges, notificationConfig, jobKey, baseUrl }) => {
  ...
  // Direct, for the same reason as in send().
  const fetchOptions = {
    method: 'POST',
    headers,
    timeout: 10000,
