...
import { syncOutboundProxy } from '../../services/http/outboundProxy.js';
 ...
 * @type {string[]}
 */
const NON_ADMIN_SETTINGS = ['demoMode', 'analyticsEnabled', 'interval', 'connectivityEnabled', 'dateFormat'];

/**
 * The date orders the interface can be pinned to, stored as the literal patterns because that is
 * what the setting's own dropdown shows.
 *
 * The empty string is the third of them and the shipped state: no order pinned, so every date is
 * written the way the reader's own language writes it, which is what the interface did before this
 * setting existed. Nothing here fills it in with a pattern - an instance that has never opened the
 * dropdown must not be moved off its users' locale by an upgrade.
 * @type {string[]}
 */
...
export default async function generalSettingsPlugin(fastify) {
  ...
  fastify.post('/', async (request, reply) => {
    ...
    if (typeof appSettings.dateFormat !== 'undefined' && !DATE_FORMATS.includes(appSettings.dateFormat)) {
      return reply
        .code(400)
        .send({ error: `dateFormat must be empty or one of: ${DATE_FORMATS.filter((it) => it !== '').join(', ')}` });
    }
    ...
    try {
      ...
      // Same promise the browser proxy makes: the new value holds from here on, without a restart.
      syncOutboundProxy(await getSettings());
