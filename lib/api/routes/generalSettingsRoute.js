...
/**
 * The two date orders the interface can render, month first - what Fredy has always done - or day
 * first. Stored as the literal patterns, because that is what the setting's own dropdown shows.
 * @type {string[]}
 */
...
export default async function generalSettingsPlugin(fastify) {
  ...
  fastify.post('/', async (request, reply) => {
    ...
    if (typeof appSettings.dateFormat !== 'undefined' && !DATE_FORMATS.includes(appSettings.dateFormat)) {
      return reply.code(400).send({ error: `dateFormat must be one of: ${DATE_FORMATS.join(', ')}` });
    }
