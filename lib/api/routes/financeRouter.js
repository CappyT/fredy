...
export default async function financePlugin(fastify) {
  ...
  fastify.get('/listing/:listingId', async (request, reply) => {
    ...
    // The model lends and taxes in euros; a price in another currency has no costing, the same answer
    // as a listing with no price at all.
    if (!Number.isFinite(price) || price <= 0 || !isDefaultCurrency(listing.currency)) {
  ...
  fastify.post('/affordability', async (request, reply) => {
    ...
    try {
      ...
      const skipped = { noPrice: 0, incompleteProfile: 0, otherCurrency: 0 };
      const items = [];
      ...
      for (const listing of listings) {
        ...
        // Scored against a budget in euros, so a price in francs would be a verdict on a wrong number.
        if (!isDefaultCurrency(listing.currency)) {
          skipped.otherCurrency++;
          continue;
        }
