...
import { isDefaultCurrency } from '../../utils/currency.js';
...
export default async function listingsPlugin(fastify) {
      ...
      thresholds == null
        ...
        : {
            ...
            result: page.result.map((listing) => ({
              ...listing,
              // The thresholds are euros; a listing in another currency has nothing to be judged by.
              affordabilityVerdict: isDefaultCurrency(listing.currency)
                ? verdictForListing(listing.price, listing.dealType, thresholds)
                : null,
            })),
          };
  ...
  fastify.get('/:listingId/image', async (request, reply) => {
    ...
    // Same access gate as every other per-listing route: a photograph is a listing's data like
    // its price is, and a guessable id must not hand out somebody else's search results.
    if (!listingStorage.userCanAccessListing(listingId, userId, isAdminFn(request))) {
      return reply.code(404).send({ message: NO_ACCESS_MESSAGE });
    }
    const stored = listingStorage.getListingImage(listingId);
    if (stored?.bytes != null) {
      // The bytes are the listing's own and never change while the row lives, so the browser can
      // hold on to them instead of asking again on every page.
      return reply
        .code(200)
        .header('Content-Type', stored.mime_type)
        .header('Cache-Control', 'private, max-age=604800')
        .send(stored.bytes);
    }
    // Nothing kept yet: the portal's own url is what the gallery has always used, whether the
    // backfill has not reached the row or the portal serves the photograph to anybody who asks.
    const listing = listingStorage.getListingById(listingId, userId, isAdminFn(request));
    if (typeof listing?.image_url === 'string' && listing.image_url.length > 0) {
      return reply.redirect(listing.image_url, 302);
    }
    return reply.code(404).send({ message: 'Listing not found' });
  });
